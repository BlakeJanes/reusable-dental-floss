...
const util = {
  ...
  // Generated files during the build time and upstream pre-installed files are different because we use different IDL files.
  ...
  // This copying in every build doesn't affect compile performance.
  ...
  // To build without much modification of upstream file, bundling mode is used. To build with this mode,
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  fixDepotTools: (options = {}) => {
    ...
    // file transitioning from untracked to a commited file. When
